...
class Converter {    
    ...
    static romanNumeralToNumber(romanNumeral) {
        ...
        for (; i < romanNumeral.length; i++) {         
            var character = romanNumeral[i];
            var nextCharacter = romanNumeral[i + 1];
            var index = romanNumerals.indexOf(character);
            var nextIndex = romanNumerals.indexOf(nextCharacter);

            if (numbers[index] >= numbers[nextIndex] || nextCharacter === undefined) {
